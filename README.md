...
- **Корзина** и **оформление заказа**: выбор дат (для отелей — расчёт по ночам),
  количество, контактные данные, **промокоды** и демо-оплата картой → подтверждение
...
- Адаптивная вёрстка с мобильным меню-бургером
...
- Тесты: `pytest` (20 тестов, изолированная in-memory БД)
