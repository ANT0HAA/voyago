  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-5 py-16 text-center">
        ...
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">Корзина пуста</h1>
        <p className="text-slate-500 dark:text-slate-400 mb-6">Добавьте рейсы, отели или туры — и они появятся здесь.</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Корзина</h1>
        <button onClick={clear} className="text-sm text-slate-400 dark:text-slate-500 hover:text-rose-600">Очистить</button>
      </div>
          ...
          return (
            <div key={it.key} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 flex gap-4 items-center">
              ...
              <div className="text-right w-28 shrink-0">
                <div className="font-bold text-slate-800 dark:text-slate-100">{money(lineTotal(it))} ₽</div>
                <button onClick={() => remove(it.key)} className="text-xs text-slate-400 dark:text-slate-500 hover:text-rose-600">удалить</button>
              </div>
            </div>
          )
        })}
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 flex items-center justify-between">
        <div>
          <div className="text-sm text-slate-500 dark:text-slate-400">Итого за {count} позиц.</div>
          <div className="text-2xl font-bold text-slate-800 dark:text-slate-100">{money(total)} ₽</div>
        </div>
