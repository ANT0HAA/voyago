...
export default function OrderSuccess() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-16 text-center">
      ...
      <h1 className="text-2xl font-bold text-slate-800 mt-4">Заказ оформлен!</h1>
          ...
          <div className="space-y-2 text-sm">
            {summary.map((s, i) => (
              <div key={i} className="flex justify-between gap-2">
                <span className="text-slate-600 truncate">{s.title} × {s.qty}</span>
                <span className="text-slate-800 font-medium whitespace-nowrap">{money(s.sum)} ₽</span>
              </div>
            ))}
          </div>
          {state?.total != null && (
            <div className="border-t border-slate-100 mt-3 pt-3 flex justify-between">
              <span className="text-slate-500 text-sm">Итого оплачено</span>
              <span className="text-lg font-bold text-slate-800">{money(state.total)} ₽</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="mt-6 flex gap-3 justify-center">
        ...
        <Link to="/" className="text-sm font-medium text-slate-600 hover:text-slate-800 px-5 py-2.5">
          На главную
        </Link>
      </div>
    </div>
