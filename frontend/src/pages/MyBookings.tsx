...
function statusInfo(b: Booking): { label: string; cls: string } {
  if (b.status !== 'confirmed') return { label: 'Отменена', cls: 'bg-rose-50 text-rose-600 dark:bg-rose-500/15' }
  const end = b.item_type === 'hotel' ? b.date_to : b.date_from
  const today = new Date(); today.setHours(0, 0, 0, 0)
  if (end && new Date(end) < today) return { label: 'Завершена', cls: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-300' }
  return { label: 'Активна', cls: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/15' }
}
...
export default function MyBookings() {
  ...
  const visible = useMemo(() => items.filter((b) =>
    tab === 'all' ? true : tab === 'cancelled' ? b.status !== 'confirmed' : b.status === 'confirmed'), [items, tab])
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 py-8">
      <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">Мои брони</h1>
      ...
      {!loading && visible.length === 0 && (
        <p className="text-slate-400 dark:text-slate-500">Здесь пока пусто.</p>
      )}
